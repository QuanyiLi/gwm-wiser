...
for tag in "${TAGS[@]}"; do
    ...
    for attempt in 1 2; do
        ...
        $PY ../gwm_tiptop/policy_server.py --select proxy --upstream-port 8765 \
            --truncate-after Place --port $PORT --log-jsonl "$OUT/served_tiptop_$tag.jsonl" \
            > "$OUT/proxy_server_$tag.log" 2>&1 &
        PS_PID=$!
        ...
        timeout -s KILL "$TASK_TIMEOUT" $PY -u place_eval.py \
            ...
            --trials "$TRIALS" --results-csv "$csv" --video-dir "$OUT/videos_tiptop_$tag" \
            --ws-port $PORT ${FAST}
        rc=$?
