...
# Selection already happened offline (run_place_score.sh argmax,
...
for tag in "${TAGS[@]}"; do
    ...
    for attempt in 1 2; do
        ...
        timeout -s KILL "$TASK_TIMEOUT" $PY -u place_eval.py \
            ...
            --trials "$TRIALS" --results-csv "$csv" --video-dir "$OUT/videos_gwm_$tag" \
            --ws-port $PORT
        rc=$?
        kill $PS_PID 2>/dev/null
        n=$( [ -f "$csv" ] && grep -c "^place_$tag," "$csv" || echo 0 )
        echo "TASKDONE gwm place_$tag attempt=$attempt rc=$rc rows=$n"
    done
done
fuser -k -s $PORT/tcp 2>/dev/null
echo "GWM PLACE ARM DONE $(date +%H:%M:%S)"
