...
near the table. Guarded by `--camera-spheres/--no-camera-spheres`.
    ...
    pixi run --manifest-path droid/tiptop/pixi.toml python -m gwm_hardware.common.build_2f140_cfg
"""
...
def build_cfg(urdf_path: Path, out_path: Path, camera_spheres: bool) -> Path:
    ...
    header = (f"# GENERATED by gwm_hardware/common/build_2f140_cfg.py "
...
def main() -> None:
    ...
    if not urdf.exists():
        raise SystemExit(f"{urdf} missing -- run `python -m gwm_hardware.common.build_2f140` first")
