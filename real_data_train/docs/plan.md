...
Everything under `real_data_train/` is organized around three pillars:
...
| # | Decision |
| --- | --- |
| D-1 | `FrankaRobotRenderer` lives under `real_data_train/`; tiptop imports read-only |
...
| # | Decision |
| --- | --- |
| D-16 | Trainer: rewrite `real_data_train/train.py` in place; `gwm_wiser/scripts/gwm_train.py` untouched |
...
| D-33 | **Per-source time-scale ranges** (amends D-30): DROID keeps s ~ log-uniform [0.5, 1.5]; **MolmoBot uses [1, 3]** (intervals 0.55–1.8 s, spans 3.0–8.9 s). Reason: PnP trajectories are scripted, smooth, and slow (17.1 s mean) — 100-sheet visual inspection showed near-static windows, and compressing them further (s < 1) yields condition ≈ target with no learning signal; the sim range therefore only stretches. `--time_scale` becomes a global override; omitted = the per-source `DEFAULT_SCALE_RANGES`. Held-out eval stays at canonical s = 1 for both sources (cross-source comparability); draws that do not fit a clip still fall back to canonical (more frequent at s→3 on short clips, by design). No re-rendering — the tree is full-FPS and the schedule is a training-time decision |
