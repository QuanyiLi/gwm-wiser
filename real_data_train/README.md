...
Data provisioning (everything lands under the gitignored `real_data_train/data/`):
...
python -m real_data_train.scripts.prepare_droid_calibration
...
python -m real_data_train.scripts.render_actions --source all

# Contact sheets from EXACT training samples (RGB / robot-only / overlay)
python -m real_data_train.scripts.visualize_dataloader --num 12 --out viz/
python -m real_data_train.scripts.visualize_dataloader \
    --sources molmoact2_droid --num 12 --out droid_viz/
```

Verify the setup with `pytest real_data_train/tests/` (no GPU needed).

## Implementation

Modules (all under `real_data_train/`, importable from the repo root). VRS/WISER code is fully deleted (plan decisions D-3/D-17).
...
Tests: `pytest real_data_train/tests/` — unit tests need no GPU and no Qwen weights (the synthetic rendered-tree fixture builds a real mp4 via imageio).
