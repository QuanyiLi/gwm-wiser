"""Per-clip video for the rendered tree (plan decisions D-27 / D-32).
...
The high-volume SIM tree (molmobot) instead uses near-lossless VP9 (D-32):
the 2026-08-08 codec shootout on real MolmoBot renders showed FFV1 is the
...
# Near-lossless VP9 for the sim rendered tree (D-32). CRF calibrated on real
# MolmoBot renders (sweep 2026-08-08, 3 streams / 723 frames @624x352):
...
def write_lossless_video(frames: np.ndarray, path, fps: float = 15.0) -> Path:
    """uint8 (T, H, W, 3) RGB -> bit-exact FFV1 MKV (real sources, D-27)."""
    return _encode(frames, path, fps, ENCODE_ARGS)


def write_near_lossless_video(frames: np.ndarray, path,
                              fps: float = 15.0) -> Path:
    """uint8 (T, H, W, 3) RGB -> near-lossless VP9 MKV (sim tree, D-32)."""
...
def verify_near_lossless(path, frames: np.ndarray,
    ...
    """Raise unless the file decodes to `frames` within the D-32 tolerance
