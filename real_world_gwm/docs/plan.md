...
**Feasibility study (2026-08-07) — the KarlP join is concrete, not fuzzy.** Verified facts:
...
- **ext1/ext2 → camera-serial assignment** was dropped in conversion; with 2 exterior cameras, try both assignments and let the overlay gate pick (wrong assignment fails reprojection decisively).
...
Recovery path (recommended, supersedes the fuzzy-join framing): **language-triple join to KarlP calibrations + per-stream overlay gate.** Expected admitted volume: cam2base coverage (~24–36k episodes) ∩ MolmoAct2 ≈ 20–30k episodes → ~1–1.5 M exterior windows at 0.5 s stride — on its own comparable to the whole run-1 MolmoBot recipe. Recorded fallbacks: (2) full self-calibration per stream (pose + focal from URDF-render alignment; no external data, strictly harder), (3) raw DROID + exact keys (loses the quality filter and LeRobot packaging, TB-scale, CC-BY) — raw DROID is **not** needed for pixels: MolmoAct2's videos are the DROID videos.
